...
def load_model(checkpoint_path: str, device: torch.device) -> torch.nn.Module:
    ...
    backbone = saved_config.get("backbone", getattr(config, "BACKBONE", "mit_b0"))
    ...
    if model_arch == "segformer":
        ...
        print(f"[INFO] Initializing Region-Aware MoE (3 Experts) for inference... (Backbone: {backbone})")
        from moe_model import RegionAwareMoE
        model = RegionAwareMoE(num_experts=3, model_name=backbone)
