...
def build_model() -> nn.Module:
    if config.MODEL_ARCH == "segformer":
        print(f"[MODEL] SegFormer  →  FPN(encoder={config.BACKBONE})")
        model = smp.FPN(
    ...
    elif config.MODEL_ARCH == "moe":
        from moe_model import RegionAwareMoE
        print(f"[MODEL] Region-Aware MoE (3 Experts)  →  Backbone={config.BACKBONE}")
        model = RegionAwareMoE(num_experts=3, model_name=config.BACKBONE)
    else:
        raise ValueError(f"Unknown MODEL_ARCH: {config.MODEL_ARCH}. Use 'segformer', 'unet', or 'moe'.")
...
class CombinedLoss(nn.Module):
    def __init__(self):
        ...
        self.lovasz = smp.losses.LovaszLoss(mode=smp.losses.BINARY_MODE)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "config": {
                    ...
                    "backbone": getattr(config, "BACKBONE", "mit_b0"),
